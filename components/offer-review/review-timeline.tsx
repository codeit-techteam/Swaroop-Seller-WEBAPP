...
import type { OfferReviewTimelineStep } from "@/types/offer-review";
...
function formatStepTime(timestamp?: string) {
  if (!timestamp) return "";
  return format(parseISO(timestamp), "MMM d, yyyy").toUpperCase();
}

function StepIcon({ status }: { status: OfferReviewTimelineStep["status"] }) {
  if (status === "completed") {
    return (
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <ol className="relative space-y-0">
        {steps.map((step, index) => {
          ...
          const isCurrent =
            step.status === "current" || step.status === "warning";
